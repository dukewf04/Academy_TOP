...
height = (ROWS + 1) * DISC_SIZE+B_HEIGHT
size = (COLUMNS * DISC_SIZE, (ROWS + 1) * DISC_SIZE+B_HEIGHT)
screen = pg.display.set_mode(size)

my_font = pg.font.SysFont('Calibri', 60)
btn = Button(pg, 0, 0, B_WIDTH, B_HEIGHT, screen)

# Для компиляции приложения в один файл
if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
    TEMP_DIR = Path(sys._MEIPASS).resolve()
else:
    TEMP_DIR = Path(sys.argv[0]).parent.resolve()
ffmpeg_path = str(TEMP_DIR / "gameover.jpg")

def draw_grid(grid):
    pg.draw.rect(screen, WHITE, (0, 0, width, B_HEIGHT))
    # Отрисовка кнопки
    btn.draw()

    for c in range(COLUMNS):
        for r in range(ROWS):
            pg.draw.rect(screen, BLUE, (c * DISC_SIZE, B_HEIGHT + r * DISC_SIZE + DISC_SIZE, DISC_SIZE, DISC_SIZE))
            pg.draw.circle(screen, WHITE, (int(c * DISC_SIZE + DISC_SIZE / 2), B_HEIGHT +
...
def main():
    while True:
        ...
        pg.draw.rect(screen, WHITE, (0, 0, width, B_HEIGHT+DISC_SIZE))
        draw_grid(grid)
        pg.display.update()
        ...
        while not new_game:
            ...
            for event in pg.event.get():
                ...
                if event.type == pg.MOUSEMOTION:
                    if btn.is_clicked(event.pos):
                        ...
                        btn.color = BLUE
                    btn.draw()
                    pg.draw.rect(screen, WHITE, (0, B_HEIGHT, width, DISC_SIZE))
                    position_x = event.pos[0]
                    if pick_random == REAL_PLAYER:
                        pg.draw.circle(screen, RED, (position_x, B_HEIGHT + int(DISC_SIZE / 2)), DISC_RADIUS)
                pg.display.update()
                    ...
                    if pick_random == REAL_PLAYER:
                        ...
                        if is_valid_position(grid, col):
                            ...
                            if search_win_move(grid, REAL_PLAYER_PIECE):
                                ...
                                screen.blit(label, (10, B_HEIGHT+20))
                                pg.display.update()
                                game_over = True
            ...
            if game_over:
                pg.time.wait(2000)
                image = pg.image.load(ffmpeg_path)
